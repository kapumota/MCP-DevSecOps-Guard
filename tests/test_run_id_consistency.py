...
def test_reports_share_global_run_id(tmp_path: Path, monkeypatch):
    ...
    skill = write_json_report(
        {"status": "PASS", "finding_counts": {"high_or_critical": 0}},
        Path(SKILL_REPORT_PATH),
        root=tmp_path,
    )
    mcp = write_json_report(
        {"status": "PASS", "finding_counts": {"high_or_critical": 0}},
        Path(MCP_AUDIT_REPORT_PATH),
        root=tmp_path,
    )
...
def test_policy_blocks_mixed_run_id_when_global_run_is_set(tmp_path: Path, monkeypatch):
    ...
    (tmp_path / SKILL_REPORT_PATH).write_text(
        json.dumps(
            {"status": "PASS", "run_id": "run-final-001", "finding_counts": {"high_or_critical": 0}}
        ),
        encoding="utf-8",
    )
    (tmp_path / MCP_AUDIT_REPORT_PATH).write_text(
        json.dumps(
            {"status": "PASS", "run_id": "otro-run", "finding_counts": {"high_or_critical": 0}}
        ),
        encoding="utf-8",
    )
