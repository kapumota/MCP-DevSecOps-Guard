...
def test_evidence_pack_creates_manifest_and_archive(tmp_path: Path):
    ...
    manifest = create_evidence_pack(root=tmp_path, output_path=Path("artifacts/evidence-test.tar.gz"))
    ...
    assert not any(entry["relative_path"] == "artifacts/evidence-manifest.json" for entry in loaded["files"])
